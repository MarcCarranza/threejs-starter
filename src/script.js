...
fontLoader.load("fonts/helvetiker_regular.typeface.json", (font) => {
  ...
  const textMaterial = new THREE.MeshMatcapMaterial({ matcap: mtcpTextureText });
  const text = new THREE.Mesh(textGeometry, textMaterial);

  // Donuts
  const donutGeometry = new THREE.TorusGeometry(.3, .2, 20, 45)
  const donutMaterial = new THREE.MeshMatcapMaterial({ matcap: mtcpTextureShiny })
  for (let i = 0; i < 100; i++) {
...
const tick = () => {
  ...
  donuts.forEach((donut, i) => {
      donut.rotation.x = (elapsedTime * 0.01) * (i * 5)
      donut.rotation.y = (elapsedTime * 0.01) * (i * 2)
  })
